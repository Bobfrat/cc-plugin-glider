...
class GliderCheck(BaseNCCheck):
    ...
    existing_varnames = util.get_required_vars()
    ...
    def existing_varname(self, var_name, check_attrs,
        ...
        sub_dict = self.existing_varnames.get(var_name, {})
        var_attrs = set(var.ncattrs())
        for attr in check_attrs:
            if attr not in var_attrs:
                messages.append("Required attribute {} for variable {} is not "
                                "present".format(attr, var_name))
                is_passing = False
                continue
            ...
            # generic catchall for non-specific attribute checks
            else:
                attr_value = getattr(var, attr, None)
                if attr_value is None:
                    messages.append("Variable {} must have attribute {}".format(var_name, attr))
